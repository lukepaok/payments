...
$title           = 'Áğïäåßîåéò Ğëçñùìşí Õğåñâáëëïõóşí Åöçìåñşí';
...
$ReportTitle2    = 'ÕĞÅÑÂÁÓÅÉÓ ÅÖÇÌÅÑÉÙÍ';
$ReportTitle3    = '';
$ReportPageTitle = '';
...
if ($Lock_Yperv>0) {
  foreach ($MasterTable as $myrow) {
    ...
    foreach ($DetailTable as $DetailRow) {
      ...
      $ADek=$DetailRow['C_A_DEK'];
      $BDek=$DetailRow['C_B_DEK'];
      ...
      foreach ($DetailRow as $key=>$value) {
        ...
        if ($Col_cat=='CB_') { $Ben_array[$BDLRE_name[$Col_desc]]=$value; }
        if ($key=='C_TAX') { $Ded_array[$BDLRE_name['D_TAXFMY']]=$value; }
        if ($Col_cat=='CD_') { $Ded_array[$BDLRE_name[$Col_desc]]=$value; }
        if ($Col_cat=='CE_') { $Erg_array[$BDLRE_name[$Col_desc]]=$value; }
      }
