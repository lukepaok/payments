...
//  echo $tmpsql;die();
  ...
  $Exsalid=$tmprow['EXSAL_ID'];
  if ($Exsalid>0) {
    $tmpsql = "SELECT SUM(EXSAD_CREDIT) AS CREDIT, SUM(EXSAD_DEBIT) AS DEBIT FROM EXTRA_SALARY_DETAIL WHERE EXSAL_ID =".$Exsalid." AND (EXSAD_MODULE='HOUR' OR EXSAD_MODULE='EXTH' OR EXSAD_MODULE='SEVE' OR EXSAD_MODULE='EBEN')";
    $tmpres =DB_query($tmpsql,$db);
    $tmprow =DB_fetch_array($tmpres);
    $Gross_efim = $tmprow['CREDIT']-$tmprow['DEBIT'];
  }
    ...
    if ($Sum_day>0) {
      ...
      if ($Efim_aa==$Over_Limit) {
        ...
        if ($Sum_7efim>=$Plafon_Duty) {
          ...
          $Sum_Part_Efims = 0;
          $Diff_small=$Plafon_Duty-($Sum_7efim-$Sum_day-$Bonus); //Παρεμβολη για να μην χανουν μια ολοκληρη εφημερια για ποσο λιγοτερο των 11 γιουρος
          if ($Diff_small<11) {
            $Apokopi = 0;
      ...
      if ($Efim_aa>$Over_Limit && $Apokopi==0 && $Done==false) {
        ...
        if ($Sum_AlEfim_p_Bonus>=$Plafon_Duty) {
          ...
          if ($Diff_small<11) {
            $Apokopi = 0;
            $Sum_Part_Efims=$Sum_AlEfim_p_Bonus - $Plafon_Duty;
          }
          else { $Apokopi = $Sum_AlEfim_p_Bonus - $Plafon_Duty; }
          $Efim_kind_o7sum=array();
          $Efim_type_o7sum=array();
          $Sum_All_o7Efim=array();
        }
