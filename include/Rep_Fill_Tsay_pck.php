...
function fillDetailTable($masterrow) {
  global $db, $DetailTable,$DetailTable_Comp, $Compute_Payroll,$Compute_Tsay, $empwhere, $emporder, $ListBox3, $Analysi_Efim, $Taktiki_Mikta;
  $Payid    = $masterrow['PAY_ID'];
  $PerYear  = $masterrow['PER_YEAR'];
  $PerMonth = $masterrow['PER_MONTH'];
  $PatId    = $masterrow['PER_MONTH'];
 $sql  = 'SELECT EMPLOYEES.EMP_ID, EMPLOYEES.EMP_PAYROLL_CODE, EMPLOYEES.EMP_LAST_NAME, EMPLOYEES.EMP_FIRST_NAME, EMPLOYEES.EMP_FATHER_NAME,
            ...
            WHERE 1=1 ';
  if ($empwhere!='') { $sql .= $empwhere; }
  if ($emporder!='') { $sql .= ' ORDER BY EMPLOYEES.'.$emporder; }
  ...
  while ($DetailRow=DB_fetch_array($DetailsResult)) {
	...
	if($PerMonth==13){$PerMonth=12;}
	if($PerMonth==14){$PerMonth=4;}
	if($PerMonth==15){$PerMonth=7;}
	$num_days = cal_days_in_month(CAL_GREGORIAN, $PerMonth, $PerYear);
	$period_start=$num_days.'-'.$PerMonth.'-'.$PerYear.' 11:59:59';
	$period_start_kom=$num_days.'-'.$PerMonth.'-'.$PerYear.' 11:59:00';
	$period_start_anadr=$num_days.'-'.$PerMonth.'-'.$PerYear;
	
    $SQL_sal = "SELECT * FROM SALARY WHERE ((SALARY.PAT_ID = ".$PatId." AND  
     EXTRACT(YEAR FROM SALARY.SAL_DATE)=".$PerYear.") OR (SALARY.PAT_ID = ".$PatId." AND SALARY.PAY_ID = ".$Payid." )) AND SALARY.EMP_ID = ".$Emp_id;
...
// [---------------------------------------------------------------- EXTRA_SALARY_DETAIL
         ...
         $tablemasterrow['KATAVLITH']= efim_paid ($Emp_id,$Payid);
            ...
            $tablemasterrow['C_ENTELOM']    = $Sum_plus_yperv+$tablemasterrow['C_ERGDED'];
            $tablemasterrow['C_DED_TAX']    = $Sum_minus_yperv;
            $tablemasterrow['C_TOTAL_DED']  = $Sum_minus_yperv+$tablemasterrow['C_ERGDED'];
            $tablemasterrow['C_PAID']       = $Sum_plus_yperv-$Sum_minus_yperv;
      ...
      elseif ($Sum_paid>0 and ($Compute_Payroll==4 || $Compute_Payroll==5 || $Compute_Payroll==6 ||$Compute_Payroll==7 || $Compute_Payroll==8 || $Compute_Payroll=='All' 
	  || $Compute_Payroll==9 || $Compute_Payroll==10 || $Compute_Payroll==12 || $Compute_Payroll==14 || $Compute_Payroll==15 || $Compute_Payroll==16 || $Compute_Payroll==17 || $Compute_Payroll==18 || $Compute_Payroll==19 || $Compute_Payroll==20 || $Compute_Payroll==21)) { $DetailTable[] = $tablemasterrow; }
      }
      //elseif($Sum_Credit==0){unset($DetailTable);}
    }
  }
  //print '<pre>';print_r($DetailTable);print '</pre>';die();
   $DetailTable_Comp=$DetailTable;
   if($Compute_Tsay ==1){
   if ($Compute_Payroll==1){
   foreach($DetailTable as $items_by_id){
   
   $items_by_id2['TWA_SAL_ID']          = $items_by_id['TWA_SAL_ID']          ;
   $items_by_id2['TWA_EMP_ID']          = $items_by_id['TWA_EMP_ID']          ;
   $items_by_id2['TWA_EMP_CODE']        = $items_by_id['TWA_EMP_CODE']        ;
   $items_by_id2['TWA_LAST_NAME']       = $items_by_id['TWA_LAST_NAME']       ;
   $items_by_id2['TWA_FIRST_NAME']      = $items_by_id['TWA_FIRST_NAME']      ;
   $items_by_id2['TWA_FATHER_NAME']     = $items_by_id['TWA_FATHER_NAME']     ;
   $items_by_id2['TWA_WORK_REL_ORIG']   = $items_by_id['TWA_WORK_REL_ORIG']   ;
   $items_by_id2['TWA_WORK_REL']        = $items_by_id['TWA_WORK_REL']        ;
   $items_by_id2['TWA_WORK_REL_REAL']   = $items_by_id['TWA_WORK_REL_REAL']   ;
   $items_by_id2['TWA_PAYTIME']         = $items_by_id['TWA_PAYTIME']         ;
   $items_by_id2['TWA_HIREDATE']        = $items_by_id['TWA_HIREDATE']        ;
   $items_by_id2['TWA_FIREDATE']        = $items_by_id['TWA_FIREDATE']        ;
   $items_by_id2['TWA_AMKA']            = $items_by_id['TWA_AMKA']            ;
   $items_by_id2['TWA_IDENTITY_NUMBER'] = $items_by_id['TWA_IDENTITY_NUMBER'] ;
   $items_by_id2['TWA_TAX_NUMBER']      = $items_by_id['TWA_TAX_NUMBER']      ;
   $items_by_id2['TWA_IKA_AM']          = $items_by_id['TWA_IKA_AM']          ;
   $items_by_id2['TWA_CATEGORY']        = $items_by_id['TWA_CATEGORY']        ;
   $items_by_id2['TWA_ERG_CATEG']       = $items_by_id['TWA_ERG_CATEG']       ;
   $items_by_id2['TWA_EIDIKOTIT']       = $items_by_id['TWA_EIDIKOTIT']       ;
   $items_by_id2['TWA_FOREAS']          = $items_by_id['TWA_FOREAS']          ;
   $items_by_id2['TWA_YPHRESIA']        = $items_by_id['TWA_YPHRESIA']        ;
   $items_by_id2['TWA_TSAY_AM']         = $items_by_id['TWA_TSAY_AM']         ;
   $items_by_id2['TWA_START_OBLIG']     = $items_by_id['TWA_START_OBLIG']     ;
   $items_by_id2['TWA_TSAY_WR']         = $items_by_id['TWA_TSAY_WR']         ;
   $items_by_id2['TWA_TSAY_CATEG']      = $items_by_id['TWA_TSAY_CATEG']      ;
   $items_by_id2['TWA_TSAY_RESP']       = $items_by_id['TWA_TSAY_RESP']       ;
   $items_by_id2['TWA_TSAY_NEW']        = $items_by_id['TWA_TSAY_NEW']        ;
   $items_by_id2['TWA_TSAY_TIME']       = $items_by_id['TWA_TSAY_TIME']       ;
   $items_by_id2['TWA_TSAY_FOR']        = $items_by_id['TWA_TSAY_FOR']        ;
   $items_by_id2['TWA_PHONE']           = $items_by_id['TWA_PHONE']           ;
   $items_by_id2['TWA_EMAIL']           = $items_by_id['TWA_EMAIL']           ;
   $items_by_id2['TWA_EMP_IS_DOCTOR']   = $items_by_id['TWA_EMP_IS_DOCTOR']   ;
   $items_by_id2['TWA_EMP_PAYTIME']     = $items_by_id['TWA_EMP_PAYTIME']     ;
   $items_by_id2['TWA_COSTC']           = $items_by_id['TWA_COSTC']           ;
   $items_by_id2['TWA_SAL_MONTH']       = $items_by_id['TWA_SAL_MONTH']       ;
   $items_by_id2['TWA_VATHMOS']         = $items_by_id['TWA_VATHMOS']         ;
   $items_by_id2['TWA_VATHMOS_ONLY']    = $items_by_id['TWA_VATHMOS_ONLY']    ;
   $items_by_id2['TWA_KLIMAKIO_ONLY']   = $items_by_id['TWA_KLIMAKIO_ONLY']   ;
   $items_by_id2['TWA_VATHM_XML_CODE']  = $items_by_id['TWA_VATHM_XML_CODE']  ;
   $items_by_id2['TWA_VATHMOS_AFT']     = $items_by_id['TWA_VATHMOS_AFT']     ;
   $items_by_id2['TWA_VATHMOS_IATR']    = $items_by_id['TWA_VATHMOS_IATR']    ;
   $items_by_id2['PLAFON_EFIM']         = $items_by_id['PLAFON_EFIM']         ;
   $items_by_id2['TWA_IBAN']            = $items_by_id['TWA_IBAN']            ;
   
   $items_by_id2['TWA_FROM_PER']        = $items_by_id['TWA_FROM_PER'];
   $items_by_id2['TWA_TO_PER']          = $items_by_id['TWA_TO_PER'] ;    
   $items_by_id2['TD_TSAY_SYNT']        = $items_by_id['TD_TSAY_SYNT']        ;
   $items_by_id2['TD_TSAY_YG']          = $items_by_id['TD_TSAY_YG']          ;
   $items_by_id2['TD_TSAY_STEG']        = $items_by_id['TD_TSAY_STEG']        ;
   $items_by_id2['TD_TSAY_PRON']        = $items_by_id['TD_TSAY_PRON']        ;
   $items_by_id2['TE_TSAY_SYNT']        = $items_by_id['TE_TSAY_SYNT']        ;
   $items_by_id2['TE_TSAY_YG']          = $items_by_id['TE_TSAY_YG']          ;
   $items_by_id2['T_INCOME']            = $items_by_id['T_INCOME']            ;
   $items_by_id2['N_PAID']              = 0;
   $items_by_id2['C_PAID']              = 0;
   $items_by_id2['M_PAID']              = 0;
  ...
  if(
	...
	($items_by_id['TWA_EMP_ID']  ==$item['TWA_EMP_ID'])   and
	($items_by_id['TWA_EMP_CODE']==$item['TWA_EMP_CODE']) and
	($items_by_id['TWA_FROM_PER']==$item['TWA_FROM_PER']) and
	($items_by_id['TWA_TO_PER']  ==$item['TWA_TO_PER'])   and
		
	($items_by_id['TWA_SAL_ID']!=$item['TWA_SAL_ID'])
	...
	$items_by_id['TWA_SAL_ID']          = 0; 
	$items_by_id['TWA_EMP_ID']          = $item['TWA_EMP_ID'];         
    $items_by_id['TWA_EMP_CODE']        = $item['TWA_EMP_CODE'] ;      
	$items_by_id['TWA_LAST_NAME']       = $item['TWA_LAST_NAME'];
	$items_by_id['TWA_FIRST_NAME']      = $item['TWA_FIRST_NAME'];
	$items_by_id['TWA_FATHER_NAME']     = $item['TWA_FATHER_NAME'];    
	$items_by_id['TWA_WORK_REL_ORIG']   = $item['TWA_WORK_REL_ORIG']; 
	$items_by_id['TWA_WORK_REL']        = $item['TWA_WORK_REL']; 
	$items_by_id['TWA_WORK_REL_REAL']   = $item['TWA_WORK_REL_REAL'];
	$items_by_id['TWA_PAYTIME']         = $item['TWA_PAYTIME']; 
	$items_by_id['TWA_HIREDATE']        = $item['TWA_HIREDATE']; 
	$items_by_id['TWA_FIREDATE']        = $item['TWA_FIREDATE']; 
	$items_by_id['TWA_AMKA']            = $item['TWA_AMKA'];
	$items_by_id['TWA_IDENTITY_NUMBER'] = $item['TWA_IDENTITY_NUMBER'];
	$items_by_id['TWA_TAX_NUMBER']      = $item['TWA_TAX_NUMBER']; 
	$items_by_id['TWA_IKA_AM']          = $item['TWA_IKA_AM']; 
	$items_by_id['TWA_CATEGORY']        = $item['TWA_CATEGORY']; 
	$items_by_id['TWA_ERG_CATEG']       = $item['TWA_ERG_CATEG']; 
	$items_by_id['TWA_EIDIKOTIT']       = $item['TWA_EIDIKOTIT'];
	$items_by_id['TWA_FOREAS']          = $item['TWA_FOREAS']; 
	$items_by_id['TWA_YPHRESIA']        = $item['TWA_YPHRESIA'] ;
	$items_by_id['TWA_TSAY_AM']         = $item['TWA_TSAY_AM'];
	$items_by_id['TWA_START_OBLIG']     = $item['TWA_START_OBLIG'];
	$items_by_id['TWA_TSAY_WR']         = $item['TWA_TSAY_WR']; 
	$items_by_id['TWA_TSAY_CATEG']      = $item['TWA_TSAY_CATEG']; 
	$items_by_id['TWA_TSAY_RESP']       = $item['TWA_TSAY_RESP']; 
	$items_by_id['TWA_TSAY_NEW']        = $item['TWA_TSAY_NEW']; 
	$items_by_id['TWA_TSAY_TIME']       = $item['TWA_TSAY_TIME']; 
	$items_by_id['TWA_TSAY_FOR']        = $item['TWA_TSAY_FOR']; 
	$items_by_id['TWA_PHONE']           = $item['TWA_PHONE'];
	$items_by_id['TWA_EMAIL']           = $item['TWA_EMAIL'];
	$items_by_id['TWA_EMP_IS_DOCTOR']   = $item['TWA_EMP_IS_DOCTOR']; 
	$items_by_id['TWA_EMP_PAYTIME']     = $item['TWA_EMP_PAYTIME']; 
	$items_by_id['TWA_COSTC']           = $item['TWA_COSTC']; 
	$items_by_id['TWA_SAL_MONTH']       = $item['TWA_SAL_MONTH']; 
	$items_by_id['TWA_VATHMOS']         = $item['TWA_VATHMOS']; 
	$items_by_id['TWA_VATHMOS_ONLY']    = $item['TWA_VATHMOS_ONLY']; 
	$items_by_id['TWA_KLIMAKIO_ONLY']   = $item['TWA_KLIMAKIO_ONLY']; 
	$items_by_id['TWA_VATHM_XML_CODE']  = $item['TWA_VATHM_XML_CODE']; 
	$items_by_id['TWA_VATHMOS_AFT']     = $item['TWA_VATHMOS_AFT']; 
	$items_by_id['TWA_VATHMOS_IATR']    = $item['TWA_VATHMOS_IATR']; 
	$items_by_id['PLAFON_EFIM']         = $item['PLAFON_EFIM'];
	$items_by_id['TWA_IBAN']            = $item['TWA_IBAN'];
	
    $items_by_id['TD_TSAY_SYNT']        += $item['TD_TSAY_SYNT'];
    $items_by_id['TD_TSAY_YG']          += $item['TD_TSAY_YG'];
    $items_by_id['TD_TSAY_STEG']        += $item['TD_TSAY_STEG'];
    $items_by_id['TD_TSAY_PRON']        += $item['TD_TSAY_PRON'];
	$items_by_id['TE_TSAY_SYNT']        += $item['TE_TSAY_SYNT'];
    $items_by_id['TE_TSAY_YG']          += $item['TE_TSAY_YG'];
	$items_by_id['T_INCOME']            += $item['T_INCOME'];
	
	
	
	$items_by_id2['TWA_SAL_ID']          = 0;
	$items_by_id2['TWA_EMP_ID']          = $items_by_id['TWA_EMP_ID']          ;
	$items_by_id2['TWA_EMP_CODE']        = $items_by_id['TWA_EMP_CODE']        ;
	$items_by_id2['TWA_LAST_NAME']       = $items_by_id['TWA_LAST_NAME']       ;
	$items_by_id2['TWA_FIRST_NAME']      = $items_by_id['TWA_FIRST_NAME']      ;
	...
	if ($items_by_id==null){continue ;}
	else{
	$items[]=$items_by_id;
	$items2[]=$items_by_id2;
	}
	//$RepData3[]=$items_by_id;
	unset($items_by_id);
	unset($items_by_id2);
  ...
  $result2=array();
  $result2 = array_unique($items2,SORT_REGULAR);
  $DetailTable_Comp=$DetailTable;
  $DetailTable=$result2;
    }
  }
