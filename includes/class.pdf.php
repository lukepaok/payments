...
class Cpdf extends FPDF {
  ...
  function selectFont($FontName) {
    $type = '';
    if (strpos($FontName,'Oblique')) { $type='I'; }
    if (strpos($FontName,'Bold')) { $type='B'; }
    $FontName ='arial';
    $this->SetFont($FontName, $type);
  }

  function newPage() { $this->AddPage(); }

  function line($x1,$y1,$x2,$y2) { FPDF::line($x1, $this->h-$y1, $x2, $this->h-$y2); }

  function addText($xb,$yb,$size,$text) {
    if ($_SESSION['GLOs']!=1) { $text=html_entity_decode($text); }
    $this->SetFontSize($size);
    $this->Text($xb, $this->h-$yb, $text);
  }

  function addInfo($label,$value) {
    if ($label=='Title') { $this->SetTitle($value); }
    if ($label=='Subject') { $this->SetSubject($value); }
    if ($label=='Creator') {
      $value=str_replace("ros.co.nz","fpdf.org",$value);
      $value=str_replace("R&OS","",$value);
      $this->SetCreator($value);
    }
    if ($label=='Author') { $this->SetAuthor($value); }
   }

   function addJpegFromFile($img,$x,$y,$w=0,$h=0) { $this->Image($img,$x,$this->h-$y-$h,$w,$h); }

   function partEllipse($x0,$y0,$astart,$afinish,$r1,$r2=0,$angle=0,$nSeg=8) { $this->ellipse($x0,$y0,$r1,$r2,$angle,$nSeg,$astart,$afinish,0); }
...
function addTextWrapAPD($xb, $yb, $w, $h, $txt, $align='J', $border=0, $fill=0) {
      $txt = html_entity_decode($txt);
      $this->x = $xb;
      $this->y = $this->h - $yb - $h;
   ...
   function addTextWrapNewLine($xb, $yb, $w, $h, $txt, $align='J', $border=0, $fill=0) {
      $txt = html_entity_decode($txt);
      $this->x = $xb;
      $this->y = $this->h - $yb - $h;
   ...
   function addTextMemo($xb, $yb, $w, $h, $txt, $align='J', $border=0, $fill=0) {
    if ($_SESSION['GLOs']!=1) { $text=html_entity_decode($text); }
      $this->x = $xb;
      $this->y = $this->h - $yb - $h;
      ...
      $txt = trim($txt);
      $this->memo_lines = $this->NbLines($w,$txt);

      $this->MultiCell($w,$h,$txt,$b,2,$align,$fill);
      $this->x=$this->lMargin;
